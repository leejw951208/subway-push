...
1. 사용자가 홈 화면에서 알림 받을 역을 검색하거나 추천 역을 선택한다.
2. 앱은 기기 위치 권한을 요청하지 않고 알림 설정 시트를 연다.
3. 선택한 `SubwayAlert.station`을 내릴 역으로 저장한다.
4. 수동 탑승 모드는 후속 기능에서 노선, 현재 역, 방향, 내릴 역 입력으로 확장한다.
...
- 위치 권한 없이 운영하는 기본 흐름에서는 사용자가 알림 받을 역을 잘못 선택하면 알림 기준도 달라진다.
